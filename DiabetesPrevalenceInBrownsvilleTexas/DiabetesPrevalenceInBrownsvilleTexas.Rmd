...
In actuality, the ages of a particular subject across all their visits were averaged and this number used to determine their age group. There derevation of stratum is not shown here because the proper proportions must be maintained in order to for weighting procedure to work. 

```{r}
# Determining sampling probability
props <- bvl2010%>%
  mutate(time="post")%>%
  gather(stratum,pop,2:7)%>%
  transmute(group=paste(tract,stratum,time),
            pop=pop,
            prop=map_dbl(pop,~.x/sum(pop)))%>%
  bind_rows(bvl2000%>%
    mutate(time="pre")%>%
    gather(stratum,pop,2:7)%>%
    transmute(group=paste(tract,stratum,time),
...
demos <- data.frame(id=unique(datamult$id),
                    group=sample(props$group,length(unique(datamult$id)),
                                   prob=props$prop,replace=T),
...
datamult <- datamult%>%
  ...
  separate(group,into=c("tract","stratum","time"),sep=" ")%>%
  mutate(id2=paste0(id,time))
         
datamult <- datamult%>%
  mutate(visitdate=ifelse(time=="pre",
                          sample(seq(as.Date("2004-01-01"),as.Date("2009-12-31"),by="day"),
                                 nrow(datamult%>%filter(time=="pre")),replace=T),
                          sample(seq(as.Date("2010-01-01"),as.Date("2018-12-31"),by="day"),
                                 nrow(datamult%>%filter(time=="post")),replace=T)),
         visitdate=as.Date(visitdate,origin="1970-01-01"))%>%
  select(id,id2,time,visitdate,tract,stratum,edu,insure,employ,diabetes)

#### Single ID Data ####
data <- datamult%>%
  group_by(id2,time,stratum,edu,tract)%>%
  summarize(diabetes=map_dbl(.x=mean(diabetes),
                           .f=~ifelse(.x==0, 0,1)),
...
Follow up has to be calcuated from the `multdata` data set because `data` drops the individual visit dates. 
...
The cohort data, was, in essence, survey data which needed to be weighted to correct for various biases in the sample. This was accomplished by comparing the sample in each stratum/tract combination to the population in that stratum/tract as estimated by the US Census for that time period. The weights are scalar multipliers applied to the sample counts in order to better approximate the population. 
...
# Stratum Weights
wj00<-Nj00%>%
  left_join(mj00,by=c("stratum","time"))%>%
  mutate(wj=Nj/mj)%>%
  select(-Nj,-mj)
...
mj10<-data%>%
  filter(str_detect(id2,"post"))%>%
  group_by(stratum)%>%
  summarize(mj=n())%>%
  mutate(time="post")

wj10<-Nj10%>%
  left_join(mj10,by=c("stratum","time"))%>%
  mutate(wj=Nj/mj)%>%
  select(-Nj,-mj)

# Aggregation
data<-data%>%
  left_join(wj00,by=c("time","stratum"))%>%
  left_join(wj10,by=c("time","stratum"))%>%
  # Puts weights into one column
  mutate(wj=map2_dbl(.x=wj.x,.y=wj.y,
                     .f=~if(is.na(.x)){return(.y)}else{return(.x)}))%>%
  select(-wj.x,-wj.y)
...
wi00<-Ni00%>%
  left_join(mi00,by=c("tract","time"))%>%
  mutate(wi=Ni/mi)%>%
  select(-Ni,-mi)
...
wi10<-Ni10%>%
  left_join(mi10,by=c("tract","time"))%>%
  mutate(wi=Ni/mi)%>%
  select(-Ni,-mi)
...
data<-data%>%
  left_join(wi00,by=c("time","tract"))%>%
  left_join(wi10,by=c("time","tract"))%>%
  mutate(wi=map2_dbl(.x=wi.x,.y=wi.y,
                     .f=~if(is.na(.x)){return(.y)}else{return(.x)}))%>%
  select(-wi.x,-wi.y)

#### wij, stratum/tract weight ####

# 2004-2009
Nij00<-bvl2000%>%
  gather(stratum,Nij,2:7)%>%
  mutate(time="pre")

mij00<-data%>%
  filter(str_detect(id2,"pre"))%>%
  group_by(tract,stratum)%>%
  summarize(mij=n())%>%
  mutate(time="pre")

wij00<-Nij00%>%
  left_join(mij00, by=c("tract","stratum","time"))%>%
  mutate(wij=Nij/mij)%>%
  select(-Nij,-mij)
...
wij10<-Nij10%>%
  left_join(mij10, by=c("tract","stratum","time"))%>%
  mutate(wij=Nij/mij)%>%
  select(-Nij,-mij)
...
data<-data%>%
  left_join(wij00,by=c("time","stratum","tract"))%>%
  left_join(wij10,by=c("time","stratum","tract"))%>%
  mutate(wij=map2_dbl(.x=wij.x,.y=wij.y,
                      .f=~if(is.na(.x)){return(.y)}else{return(.x)}))%>%
  select(-wij.x,-wij.y)
...
Three seperate models were calculated for this data. The first was a crude estimate of diabetes prevelence simply calculated by $prevalence=\frac{cases}{observations}$. The second model took into account the weighting procedure. The third "spatially smoothed" the estimates (using Dr. Cici Bauer's Method) meaning that areas that were geographically closer to one another were assumed to be more similar than ones further away. A spatial model was only calculated for the later time period due to the higher number of tracts sampled. 

```{r}
#### Prepare data ####
# Data with diabetic observations
# Not a problem in simulated data but important with survey data
svydata00<-data%>%
  filter(str_detect(id2,"pre")& !is.na(diabetes))%>%
  left_join(Nij00,by=c("tract","stratum","time"))
svydata10<-data%>%
  filter(str_detect(id2,"post")& !is.na(diabetes))%>%
  left_join(Nij10,by=c("tract","stratum","time"))

# Strata population for existing combinations
N.j00<-svydata00%>%
  distinct(stratum, tract, .keep_all = TRUE)%>% 
  group_by(stratum)%>% 
  summarize(fpc = sum(Nij))
N.j10<-svydata10%>%
  distinct(stratum, tract, .keep_all = TRUE)%>% 
  group_by(stratum)%>% 
  summarize(fpc = sum(Nij))

# Tract population for existing combinations
N.i00<-svydata00%>%
  group_by(stratum, tract)%>%
  distinct(stratum,tract,.keep_all=T)%>%
  group_by(tract)%>%
  summarize(fpc=sum(Nij))
N.i10<-svydata10%>%
  group_by(stratum, tract)%>%
...
```{r}
#### Post statification with Survey Package ####

# Create design objects
des00 <- svydesign(id= ~1, stratum=~tract,weights=~wij,data=svydata00, fpc=~fpc)
des10 <- svydesign(id= ~1, stratum=~tract,weights=~wij,data=svydata10, fpc=~fpc)

# Format Nj objects
pop.types00<-N.j00%>%rename(Freq=fpc)
pop.types10<-N.j10%>%rename(Freq=fpc)

# Alter survey design object
psdes00<-postStratify(des00,~stratum,pop.types00)
psdes10<-postStratify(des10,~stratum,pop.types10)

results<-results%>%
  mutate(tract=as.character(tract))%>%
  # Crude prevalence
  left_join(y.i00,by="tract")%>%
  left_join(m.i00,by="tract")%>%
  rename(y00=y,m00=m)%>%
  left_join(y.i10,by="tract")%>%
  left_join(m.i10,by="tract")%>%
  rename(y10=y,m10=m)%>%
  mutate(CrudeE00=y00/m00,CrudeE10=y10/m10,
         CrudeS00=CrudeE00*(1-CrudeE00)/m00,
         CrudeS10=CrudeE10*(1-CrudeE10)/m10)%>%
  select(tract,y00,m00,CrudeE00,CrudeS00,y10,m10,CrudeE10,CrudeS10)%>%
  # Weighted prevalence without post stratification
  left_join(svyby(~diabetes, ~tract, svymean,design=des00,keep.names = F),by="tract")%>%
  rename(SvyWijE00=diabetes,SvyWijS00=se)%>%
  left_join(svyby(~diabetes, ~tract, svymean,design=des10,keep.names = F),by="tract")%>%
  rename(SvyWijE10=diabetes,SvyWijS10=se)%>%
  # Weighted prevalence with post stratification
  left_join(svyby(~diabetes, ~tract, svymean,design=psdes00,keep.names = F),by="tract")%>%
  rename(SvyPSE00=diabetes,SvyPSS00=se)%>%
  left_join(svyby(~diabetes, ~tract, svymean,design=psdes10,keep.names = F),by="tract")%>%
  rename(SvyPSE10=diabetes,SvyPSS10=se)
  
#### Calculation of .* values ####

# .* is the weighted version of . 
# Correlate to effective sample sizes and cases

# Calculate stratum prevalence
p.j00<-svyby(~diabetes, ~stratum, svymean, design=psdes00,keep.names=F)
names(p.j00)[2] <- "p_stratum"
p.j10<-svyby(~diabetes, ~stratum, svymean, design=psdes10,keep.names=F)
names(p.j10)[2] <- "p_stratum"

results<-results%>%
  # Calculate residual, e
  left_join(svydata00%>%
              left_join(p.j00,by=c("stratum"))%>%
              mutate(e2.00 = (diabetes - p_stratum)^2)%>%
              group_by(tract)%>%
              summarize(RSS00=sum(e2.00))%>%
              left_join(N.i00)%>%
              left_join(m.i00)%>%
              left_join(y.i00)%>%
              mutate(var.p00 = ((fpc-m)/fpc)*(RSS00/(m*(m-1)))),
            by="tract")%>%
  mutate(m.star00=SvyPSE00*(1-SvyPSE00)/var.p00,
                 y.star00=m.star00 * SvyPSE00)%>%
  select(-RSS00,-fpc,-m,-y,-var.p00)%>%
  left_join(svydata10%>%
              left_join(p.j10,by=c("stratum"))%>%
              mutate(e2.10 = (diabetes - p_stratum)^2)%>%
              group_by(tract)%>%
              summarize(RSS10=sum(e2.10))%>%
              left_join(N.i10)%>%
              left_join(m.i10)%>%
              left_join(y.i10)%>%
              mutate(var.p10 = ((fpc-m)/fpc)*(RSS10/(m*(m-1)))),
            by="tract")%>%
  mutate(m.star10=SvyPSE10*(1-SvyPSE10)/var.p10,
         y.star10=m.star10 * SvyPSE10)%>%
  select(-RSS10,-fpc,-m,-y,-var.p10)
...
# MAke sure order of shapePoly is same as data
shp@data$tract<-as.numeric(as.character(shp@data$NAME10))
results<-data.frame(tract=unique(c(focus00,focus10)))


data.inla <-left_join(shp@data, results%>%mutate(tract=as.numeric(tract)),  by = c("tract"))

# Y model 
formula <- y10 ~ 1 + f(ID, model="bym", graph="MappingObjects/focusmap10.adj", hyper=list(prec.unstruct=list(prior="loggamma",param=c(0.5, 0.008)), prec.spatial=list(prior="loggamma",param=c(0.5, 0.008))))

mod <- inla(formula, family="binomial", data=data.inla, Ntrials=m10, control.predictor=list(compute=TRUE, link=1))

# Y* model

formula <- y.star10 ~ 1 + f(ID, model="bym", graph="MappingObjects/focusmap10.adj", hyper=list(prec.unstruct=list(prior="loggamma",param=c(0.5, 0.008)), prec.spatial=list(prior="loggamma",param=c(0.5, 0.008))))

mod2 <- inla(formula, family="binomial", data=data.inla, Ntrials=m.star10, control.predictor=list(compute=TRUE, link=1))
...
results<-results%>%
  left_join(data.inla%>%
              select(tract)%>%
              mutate(tract=as.character(tract),
                     InlaE10 = mod$summary.fitted.values[, 1],
                     Inla.starE10 = mod2$summary.fitted.values[, 1],
                     InlaS10=mod$summary.fitted.values[,2],
                     Inla.starS10=mod2$summary.fitted.values[,2]),
            by="tract")%>%
  # for joining with mapping, id is the region name
  select(id=tract,
         # Crude 2000
         y00,m00,CrudeE00,CrudeS00,
         # Weighted 2000
         SvyWijE00,SvyWijS00,
         # Post stratified 2000
         y.star00,m.star00,SvyPSE00,SvyPSS00,
         # Crude 2010
         y10,m10,CrudeE10,CrudeS10,
         # Weighted 2010
         SvyWijE10,SvyWijS10,
         # Post stratified 2010
         y.star10,m.star10,SvyPSE10,SvyPSS10,
         # Spatial smoothing 2010
         InlaE10,InlaS10,
         # Weighted spatial smoothing 2010
         Inla.starE10,Inla.starS10)%>%
  arrange(id)
...
#### Stratum Prevalence ####
strata<-p.j00%>%
  left_join(mj00,by="stratum")%>%
  bind_rows(p.j10%>%
              left_join(mj10,by="stratum"))%>%
  mutate(time=map_chr(.x=time,~ifelse(.x=="pre","2004-2009","2010-2018")))
```
