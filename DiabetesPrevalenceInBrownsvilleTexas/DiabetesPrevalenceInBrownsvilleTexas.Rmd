...
demos <- data.frame(id=unique(data$id),
                    gender=sample(c("male","female"),length(unique(data$id)),replace=T),
                    tract=sample(focus00,length(unique(data$id)),replace=T),
                    edu=sample(c("Less than High School","High School / GED","Post-Secondary School"),
                               length(unique(data$id)),replace=T))
