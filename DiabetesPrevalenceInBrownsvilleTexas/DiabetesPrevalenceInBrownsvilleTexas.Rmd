...
svydata00<-data%>%
  filter(str_detect(RRID2,"pre")& !is.na(diabetic))%>%
  left_join(Nij00,by=c("tract","strata","time"))
svydata10<-data%>%
  filter(str_detect(RRID2,"post")& !is.na(diabetic))%>%
  left_join(Nij10,by=c("tract","strata","time"))

# Strata population for existing combinations
N.j00<-svydata00%>%
  distinct(strata, tract, .keep_all = TRUE)%>% 
  group_by(strata)%>% 
...
N.i00<-svydata00%>%
  group_by(strata, tract)%>%
  distinct(strata,tract,.keep_all=T)%>%
  group_by(tract)%>%
  summarize(fpc=sum(Nij))
N.i10<-svydata10%>%
  group_by(strata, tract)%>%
...
des00 <- svydesign(id= ~1, strata=~tract,weights=~wij,data=svydata00, fpc=~fpc)
des10 <- svydesign(id= ~1, strata=~tract,weights=~wij,data=svydata10, fpc=~fpc)
...
psdes00<-postStratify(des00,~strata,pop.types00)
psdes10<-postStratify(des10,~strata,pop.types10)
...
results<-results%>%
  ...
  left_join(svyby(~diabetic, ~tract, svymean,design=des00,keep.names = F),by="tract")%>%
  rename(SvyWijE00=diabetic,SvyWijS00=se)%>%
  left_join(svyby(~diabetic, ~tract, svymean,design=des10,keep.names = F),by="tract")%>%
  rename(SvyWijE10=diabetic,SvyWijS10=se)%>%
  # Weighted prevalence with post stratification
  left_join(svyby(~diabetic, ~tract, svymean,design=psdes00,keep.names = F),by="tract")%>%
  rename(SvyPSE00=diabetic,SvyPSS00=se)%>%
  left_join(svyby(~diabetic, ~tract, svymean,design=psdes10,keep.names = F),by="tract")%>%
  rename(SvyPSE10=diabetic,SvyPSS10=se)
...
p.j00<-svyby(~diabetic, ~strata, svymean, design=psdes00,keep.names=F)
names(p.j00)[2] <- "p_stratum"
p.j10<-svyby(~diabetic, ~strata, svymean, design=psdes10,keep.names=F)
names(p.j10)[2] <- "p_stratum"

results<-results%>%
  # Calculate residual, e
  left_join(svydata00%>%
              left_join(p.j00,by=c("strata"))%>%
              mutate(e2.00 = (diabetic - p_stratum)^2)%>%
              group_by(tract)%>%
  ...
  left_join(svydata10%>%
              left_join(p.j10,by=c("strata"))%>%
              mutate(e2.10 = (diabetic - p_stratum)^2)%>%
              group_by(tract)%>%
...
#### Stratum Prevalence ####
strata<-p.j00%>%
  left_join(mj00,by="strata")%>%
  bind_rows(p.j10%>%
              left_join(mj10,by="strata"))%>%
